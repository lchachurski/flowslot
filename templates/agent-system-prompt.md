...
- **`needs_login: true` — Claude's OAuth token has expired on that slot.** Regardless of `status`, Claude cannot process any input until someone re-authenticates it. Do NOT report "idle" or "ready" or attempt `inject_message`; the message would just sit unprocessed. Instead say something like: "Claude on `<slot>` is signed out — the OAuth token expired. You'll need to log in from a terminal (`slot claude attach --slot <slot>`, then `/login`) before it can do anything." If the user asks you to send a message anyway, refuse politely and repeat the login instruction. This overrides the normal status-based reporting.
